...
$(error "No cargo in $(PATH), install rust if you want to build with it")
endif

ifeq (, $(shell which go))
$(error "No go in $(PATH), install go if you want to build with it")
endif

ifeq (, $(shell which gox))
$(error "No gox in $(PATH), install gox if you want to cross-compile with it")
endif

ifeq (, $(shell which docker))
$(error "No docker in $(PATH), install docker if you want to create images")
endif
...
docker:
	...
	-IMAGE=cpud:latest ./TESTDOCKERDECPU
